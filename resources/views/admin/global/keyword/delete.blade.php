...
<!-- Modal Delete -->
<div id="modal-delete-keyword" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-keyword">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteKeyword: "/admin/keyword/delete",
        }
        let idIntro = null;

        // event xoa
        $('#idTable_keyword tbody').on("click", ".delete-keyword", function () {
            idIntro = $(this).data("id");
        })

        $('#modal-delete-keyword').on('click', '#delete-keyword', function () {
            let data = {
                'id': idIntro,
            }
            let result = ajaxQuery(config.urlDeleteKeyword, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableKeyword()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-keyword [data-dismiss=modal]").trigger({type: "click"});
