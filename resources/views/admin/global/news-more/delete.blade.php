...
<!-- Modal Delete -->
<div id="modal-delete-news-more" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-news-more">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteNewsMore: "/admin/news_more/delete",
        }
        let idIntro = null;

        // event xoa
        $('#idTable_news_more tbody').on("click", ".delete-news-more", function () {
            idIntro = $(this).data("id");
        })

        $('#modal-delete-news-more').on('click', '#delete-news-more', function () {
            let data = {
                'id': idIntro,
            }
            let result = ajaxQuery(config.urlDeleteNewsMore, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableNewsMore()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-news-more [data-dismiss=modal]").trigger({type: "click"});
