...
<!-- Modal Delete -->
<div id="modal-delete-document" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-document">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteDocument: "/admin/document/delete",
        }
        let idIntro = null;

        // event xoa
        $('#idTable_document tbody').on("click", ".delete-document", function () {
            idIntro = $(this).data("id");
        })

        $('#modal-delete-document').on('click', '#delete-document', function () {
            let data = {
                'id': idIntro,
            }
            let result = ajaxQuery(config.urlDeleteDocument, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableDocument()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-document [data-dismiss=modal]").trigger({type: "click"});
