...
<!-- Modal Delete -->
<div id="modal-delete-related" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-related">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteRelated: "/admin/related/delete",
        }
        let idRelated = null;

        // event xoa
        $('#idTable_related tbody').on("click", ".delete-related", function () {
            console.log($(this).data("id"))
            idRelated = $(this).data("id");
        })

        $('#modal-delete-related').on('click', '#delete-related', function () {
            let data = {
                'id': idRelated,
            }
            let result = ajaxQuery(config.urlDeleteRelated, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableRelated()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-related [data-dismiss=modal]").trigger({type: "click"});
