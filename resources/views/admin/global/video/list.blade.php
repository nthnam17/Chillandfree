...
?>
<style>
    #table_paginate_video {
        margin-top: 15px;
    }
</style>
<div class="card mb-0">
    <div class="main-search-news">
        <div class="row" id="searchFormVideo">
            <div class="col-xs-2 col-md-3">
                <input class="form-control" group="data" name="search_name_video" type="text" autocomplete="off" spellcheck="false" placeholder="Tiêu đề">
            </div>
            <div class="col-xs-2 col-md-3">
            ...
            <div class="col-xs-2 col-md-3">
                <button class="btn btn-outline-primary btn-custom" id="btnSearchVideo">
                    <i class="fa fa-search" aria-hidden="true"></i> Tìm kiếm
                </button>
                <button class="btn btn-outline-success btn-custom" id="btnResetVideo">
    ...
    <div class="d-flex justify-content-end mb-3">
        <div class="card-header-actions">
            <button class="btn btn-outline-primary btn-custom add-modal-video" data-toggle="modal" data-target="#modal-form-video">
                <i class="fa fa-plus" aria-hidden="true"></i> Thêm mới
            </button>
        </div>
    </div>
    <div id="idTable_video">
        ...
        <div class="datatables__info_wrap">
            ...
            <div class="paging_simple_numbers pull-right" id="table_paginate_video"></div>
...
<script>
    $(document).ready(function () {
        const config = {
            urlGetVideo: "/admin/videos/list",
        };

        let pageIndex = 1;
        let lastPage = 0;

        $('#modal-lookup-tab').on('click', '#nav-video-tab', function () {
            pageIndex = 1;
            window.loadDataTableVideo();
        });

        // load data thông tin
        window.loadDataTableVideo = function() {
            $('#idTable_video tbody tr').remove();

            let lookup_id = $("input[name='id-intro']").val();
            let data = {
                id: lookup_id,
                title: $("#searchFormVideo input[name='search_name_video']").val(),
                status: $("#searchFormVideo select[name='search_status']").val(),
                page: pageIndex
            }

            let result = ajaxQuery(config.urlGetVideo, data, 'GET');
            ...
            $.each(lstData, function(index, item){
                ...
                let newRow = "<tr id='"  + item.id + "'>" +
                    ...
                    "<td>" + status + "</td>" +
                    "<td>";
                newRow += '<a title="Chỉnh sửa" class="btn btn-sm active edit-video" href="" name="id" style="color: #4B49AC" data-toggle="modal" data-target="#modal-edit-video" data-id="' + item.id + '"><i class="mdi mdi-pencil"></i></a>' +
                    '<a title="Xóa" class="btn btn-sm active delete-video" href="" name="id" style="color: #f5302e" data-toggle="modal" data-target="#modal-delete-video" data-id="' + item.id + '"><i class="mdi mdi-delete"></i></a>' +
                    "</td>" +
                    "</tr>";
                $('#idTable_video tbody').append(newRow);
            });

            paginationVideo(result);

        }


        $('.add-modal-video').click(function () {
            clearData();
        })

        $('#btnResetVideo').click(function () {
            $("#searchFormVideo input[name='search_name_video']").val('');
            $("#searchFormVideo select[name='search_status']").val('');
            pageIndex = 1;
        ...
        function clearData() {
            $("#modal-form-video input[name='title']").val('');
            $("#modal-form-video input[name='source']").val('');
            $("#modal-form-video select[name='status']").val(1);
            $("#modal-form-video input[name='priority']").val('');

            $("#modal-form-video .show-invalid").remove();
        }
        ...
        function paginationVideo(data) {
            ...
            if(data.data.last_page == 1){
                $('#table_paginate_video').html(htmlPanigation);
            } else {
            ...
            $("#table_paginate_video nav ul li a").click(function (e) {
