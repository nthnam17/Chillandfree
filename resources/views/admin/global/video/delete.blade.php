...
<!-- Modal Delete -->
<div id="modal-delete-video" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-video">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteVideo: "/admin/videos/delete",
        }
        let idIntro = null;

        // event xoa
        $('#idTable_video tbody').on("click", ".delete-video", function () {
            idIntro = $(this).data("id");
        })

        $('#modal-delete-video').on('click', '#delete-video', function () {
            let data = {
                'id': idIntro,
            }
            let result = ajaxQuery(config.urlDeleteVideo, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableVideo()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-video [data-dismiss=modal]").trigger({type: "click"});
