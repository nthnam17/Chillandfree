...
<!-- Modal Delete -->
<div id="modal-delete-battles" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-battles">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteBattles: "/admin/battles/delete",
        }
        let idBattles = null;

        // event xoa
        $('#idTable_battles tbody').on("click", ".delete-battles", function () {
            console.log($(this).data("id"))
            idBattles = $(this).data("id");
        })

        $('#modal-delete-battles').on('click', '#delete-battles', function () {
            let data = {
                'id': idBattles,
            }
            let result = ajaxQuery(config.urlDeleteBattles, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableBattles()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-battles [data-dismiss=modal]").trigger({type: "click"});
