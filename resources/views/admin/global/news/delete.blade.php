...
<!-- Modal Delete -->
<div id="modal-delete-news" class="modal fade" role="dialog">
    <div class="modal-dialog " style="width: 500px; margin-top:15%">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer modal-footer-delete">
                <button type="button" class="btn btn-dagerdel my-btn-default" id="delete-news">
...
<script>
    $(document).ready(function () {
        const config = {
            urlDeleteNews: "/admin/news/delete",
        }
        let idIntro = null;

        // event xoa
        $('#idTable_news tbody').on("click", ".delete-news", function () {
            idIntro = $(this).data("id");
        })

        $('#modal-delete-news').on('click', '#delete-news', function () {
            let data = {
                'id': idIntro,
            }
            let result = ajaxQuery(config.urlDeleteNews, data, 'POST');
            if (result.code == 200) {
                window.loadDataTableNews()
                //notify success
                swalSuccess(result.message);
                //close modal
                $("#modal-delete-news [data-dismiss=modal]").trigger({type: "click"});
